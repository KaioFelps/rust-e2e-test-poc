    ...
    B: 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        let errors = session.remove(options.sessions_errors_key).map(|errors| {
        ...
        let before_prev_url = session
            .get::<String>(options.sessions_prev_req_key)
            .unwrap_or(None)
            .unwrap_or("/".into());

        let prev_url = session
            .get::<String>(options.sessios_curr_req_key)
            .unwrap_or(None)
            .unwrap_or("/".into());
        ...
        Box::pin(async move {
            ...
            if let Err(err) = session.insert(
                options.sessions_errors_key,
                optional_errors.unwrap_or_default(),
            ) {
                error!("Failed to add errors to session: {}", err);
            }

            if let Err(err) = session.insert(options.sessions_prev_req_key, prev_url) {
                error!("Failed to update session previous request URL: {}", err);
            };

            if let Err(err) = session.insert(options.sessios_curr_req_key, curr_url) {
