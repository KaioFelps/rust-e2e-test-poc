...
export abstract class PaginationUtils {
    ...
    public static getCalculatedPaginationData(props: GetPaginationPropsArgs): CalculatedPaginationData {
        const { isFirstPageVisible, isLastPageVisible, ...boundaries } = PaginationUtils.getPaginationProps(props);
        const { calculatedPages } = PaginationUtils.generatePaginationButtons(boundaries);
    ...
    private static getPaginationProps({
        ...
        let maxLeft = currentPage - Math.floor(this.visibleButtons / 2);
        let maxRight = currentPage + Math.floor(this.visibleButtons / 2);
        ...
        if (maxRight >= totalOfPages) {
            maxLeft = totalOfPages - (this.visibleButtons - 1);
            maxRight = totalOfPages;
        }
      
        if (totalOfPages < this.visibleButtons + 1) {
            maxLeft = 1;
            maxRight = totalOfPages;
        }
